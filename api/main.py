...
@app.post("/upload")
async def upload_document(file: UploadFile = File(...), grade: str = "1"):
    ...
    try:
        ...
        supabase.table("documents").update({"content": markdown_content, "status": "ready"}).eq("pdf_url", file_url).execute()
        return {"status": "success", "data": {**doc_data, "content": markdown_content}}
    except Exception as e:
